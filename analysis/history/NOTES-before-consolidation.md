...
## Evidence audit correction (2026-09-12)

Supersedes the earlier fixed 12-byte envelope / field-A / field-B conclusions and GUI-only
“wall” claim. The middle copy in `data_obtain` uses `param_4`, not a fixed 4-byte count.
The 12/20-byte checks apply to the returned record size in two callers of the same reader.
Internal FIFO framing has not been shown to be USB framing; a USB checksum is not established
by the channel-level `CHECKSUM_DISABLED` flag. See PROTOCOL.md §3.1 for the source audit.
Next bounded investigation: trace outgoing construction toward `WriteFile`. No new probes sent.

## Outgoing WriteFile trace (2026-09-13)

Assembly tracing with objdump found `FUN_1006e180` -> `FUN_1006a420` -> WriteFile
(import 0x1007f034, call 0x1006a473). The wrapper sends LE16(N), LE16(N XOR 0x51E6),
then N unchanged body bytes, totaling N+4. The XOR constant is initialized in a subobject
at context+4; subobject+0x4024 equals send-context+0x4028. This supersedes the 32-bit
length-prefix hypothesis. Receive parsing corroborates the length XOR check.

A 17-byte cIoctl body places opcode 0x11 at body+4 and sequence at body+6; its prefix
is 11 00 f7 51. Body bytes 10–11 are not explicitly initialized in the inspected caller
range. See PROTOCOL.md §3 and analysis/disassembly/ for evidence and limitations.
No device commands sent. Next: trace cOpenDevice body construction and initialization.

Analyzed DLL SHA-256: 2710726fdd174f32b5e703e310cfce047a663451edbb3e35ca0c8eeff6929413.

## cOpenDevice construction (2026-09-13)

Found FUN_1003ea00, sending a 12-byte body through FUN_1006e180 at 1003ea9c.
WriteFile buffer: `0c 00 ea 51 01 00 00 00 03 00 SS 00 00 00 ?? ??`.
SS is the nonzero sequence byte; final two bytes lack explicit initialization in the
inspected function. Saved assembly and global constants in analysis/disassembly/.
The observed caller invokes FUN_1003e820 (cJumpToFirmware, 0x0103) immediately before
cOpenDevice, so open is not the first transaction on this path. See PROTOCOL.md §3.
No commands sent. Next: preceding initialization and response handling.

## Ghidra installed and used (2026-09-13)

Ghidra 12.1.2 Snap headless extraction succeeded against the existing project, read-only.
Saved nine decompiled functions and reference indexes using ExtractOpenSequence.java.
Confirmed opening flow: hardware initialization -> cJumpToFirmware (accepts status 0/7)
-> cOpenDevice (accepts 0; 0x020a explicitly means missing vehicle-connector voltage)
-> cGetBoardInfo. See PROTOCOL.md §10 for evidence and remaining gaps. No hardware I/O.

## Deep Ghidra pass (2026-09-13)

Major correction: the kernel driver sends vendor setup `40 db 01 00 00 00 00 00` on
file creation and value zero on file cleanup. This was absent from previous tty probes.
Bulk read/write forwards request memory unchanged. Saved 55 kernel functions and WDF labels.

Extracted all 47 direct control-sender functions with assembly. Corrected register signatures
resolve FIFO records as [u32 N][body][u32 N]; the old “checksum field” was internal bookkeeping.
Response matching compares routing word and sequence, not opcode. Resolved startup pointer to
pthread_create and receive worker 1006e480. Mapped ordinary inbound/outbound data, normal filters,
periodic messages, discovery echo and board-info path, and ISO15765 host-side receive reassembly.

PROTOCOL.md was consolidated to replace earlier contradictory hypotheses; its prior version is
archived in analysis/history/. Kernel 0xdb behavior and correct framing still need hardware
validation. No device commands sent. See analysis/REPRODUCE.md for Ghidra workflows.
